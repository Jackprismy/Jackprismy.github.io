...
// タイムアウト付き get(): 指定ms以内に応答なければキャッシュまたは空で解決
function getWithTimeout(ref, ms = 5000) {
  return Promise.race([
    ref.get(),
...
// コレクションのタイムアウト付き get()
function colGetWithTimeout(ref, ms = 5000) {
  return Promise.race([
    ref.get(),
    new Promise((_, reject) =>
      setTimeout(() => reject(new Error('Firestore col timeout')), ms)
    )
  ]);
}
...
async function loadSettings() {
  try {
    const snap = await getWithTimeout(dataDoc('settings'));
...
async function loadProgress() {
  try {
    const snap = await getWithTimeout(dataDoc('progress'));
    if (snap.exists) {
      const d = snap.data();
...
async function loadMissed() {
  missedWords = {};
  _missedChunkIds = new Set();
  try {
    const snaps = await colGetWithTimeout(missedCol());
    snaps.forEach(d => {
      const cid = parseInt(d.id.replace('chunk_', ''));
      _missedChunkIds.add(cid);
      Object.assign(missedWords, d.data().words || {});
    });
  } catch (e) { console.error('loadMissed:', e); }
}
// saveMissed: 既存doc一覧をメモリで持ち、再読み込み不要にする
let _missedChunkIds = new Set();   // Firestore に存在するチャンクID

async function saveMissed() {
  // ローカルの missedWords からチャンクを構築
  const chunks = {};
  ...
  try {
    ...
    // 今回生成されなかった（削除対象の）チャンクIDを消す
    for (const cid of _missedChunkIds) {
      if (!chunks[cid]) batch.delete(missedCol().doc(`chunk_${cid}`));
    }
    // 書き込み
    ...
    await batch.commit();
    // 保存後に既存IDを更新
    _missedChunkIds = new Set(Object.keys(chunks).map(Number));
...
async function loadCurrentSession() {
  try {
    const snap = await getWithTimeout(dataDoc('session'));
...
async function loadCustomCourse() {
  try {
    const snap = await getWithTimeout(dataDoc('customCourse'));
...
async function loadWordFile(num) {
  ...
  try {
    const res = await fetch(`word${num}-2.json`);
...
async function finishSession() {
  // セッション終了処理はバックグラウンドで行い、結果画面を即表示
  if (studyMode !== 'custom') clearCurrentSession().catch(console.error);
  showResultScreen();
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // カスタムコース進行ボタン（アクティブ時：中断中なら再開、なければ次のバッチ）
  document.getElementById('btn-custom-course').addEventListener('click', () => {
    if (!customCourse) return;
    const hasBatchInProgress = customCourse.currentBatch &&
      customCourse.currentBatch.length > 0 &&
      customCourse.batchIdx < customCourse.currentBatch.length;
