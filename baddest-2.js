...
// ローディング画面のステータスメッセージをフェード切り替えで更新
function setLoadingStatus(status, step = '') {
  ...
  const update = () => {
    ...
    if (elStep) elStep.classList.remove('fade');
  };

  // 現在表示中なら一瞬フェードアウトして切り替え
  if (!document.getElementById('loading-overlay').classList.contains('hidden')) {
    elStatus.classList.add('fade');
    if (elStep) elStep.classList.add('fade');
...
async function onUserLoggedIn() {
  ...
  // 4つのFirestore読み込みを並列実行（直列より大幅に高速化）
...
async function startCustom() {
  ...
  showLoading('カスタムセッション準備中', `${start}〜${end}番から${count}問を選んでいます...`);
