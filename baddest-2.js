...
async function markCorrect() {
  ...
  await saveProgress();  // ← await で確実に保存してから次へ
  await nextWord();
}
...
async function onCorrectBtn() {
  ...
  await saveProgress();  // ← await で確実に保存
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ページ離脱時は saveProgress が間に合わない場合があるが、
  // 問題回答ごとに await saveProgress() しているため
  // 「最後に回答した問題まで」は必ず保存済み。
  // ここでは念のため追加保存を試みる。
